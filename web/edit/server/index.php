...
if (!empty($_POST['save'])) {
    ...
    if (empty($_SESSION['error_msg'])) {
        if ($_POST['v_upgrade_send_notification_email'] != $_SESSION['UPGRADE_SEND_NOTIFICATION']) {
            ...
            exec (HESTIA_CMD."v-change-sys-config-value UPGRADE_SEND_NOTIFICATION ".escapeshellarg($_POST['v_upgrade_send_notification_email']), $output, $return_var);
            check_return_code($return_var,$output);
            unset($output);
    ...
    // Update save upgrade log setting
    if (empty($_SESSION['error_msg'])) {
        if ($_POST['v_upgrade_save_log'] != $_SESSION['UPGRADE_SAVE_LOG']) {
            if ($_POST['v_upgrade_save_log'] == 'on') { $_POST['v_upgrade_save_log'] = 'true'; } else { $_POST['v_upgrade_save_log'] = 'false'; }
            exec (HESTIA_CMD."v-change-sys-config-value UPGRADE_SAVE_LOG ".escapeshellarg($_POST['v_upgrade_save_log']), $output, $return_var);
            check_return_code($return_var,$output);
            unset($output);
            $v_upgrade_save_log_adv = 'yes';
        }
    }
